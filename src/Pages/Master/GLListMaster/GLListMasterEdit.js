...
const GLListMasterEdit = () => {
 ...
 const user_id = user_info.user_id
  ...
  const [singleGlInfo, setSingleGlInfo] = useState({})
  ...
  function addNewGLList() {
    ...
    let current_info = getOldLogInfo(singleGlInfo ? singleGlInfo.log_info : null)
    current_info.push({
      ...
      plant_code: values.plant,
      vendor_customer_type: (() => {
        const codes = String(values.gltype).split(',')
        const names = codes.map(code => {
          const found = vendorCustomerTypes.find(vct => String(vct.definition_list_code) === String(code.trim()))
