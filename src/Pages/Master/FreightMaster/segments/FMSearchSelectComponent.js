 ...
 isMultiple
  ...
  useEffect(() => {
   ...
   if (search_type == 'delivery_track_shipment_routes') {
    ...
    } else if(search_type == 'invoice_report_inco_term'){
  ...
  if (search_type == 'invoice_report_vehicle_number') {
  ...
  } else if (search_type == 'invoice_report_customer_name') {
    console.log(search_data,'-invoice_report_customer_name-search_data')
    let sp_tsno_array = []
    search_data.map(({ customer_info_updated }) => {
      if (sp_tsno_array.indexOf(customer_info_updated.CustomerCode) === -1) {
        sp_tsno_array.push(customer_info_updated.CustomerCode)
        option.push({ value: customer_info_updated.CustomerCode, label: `${customer_info_updated.CustomerName} : ${customer_info_updated.CustomerCode}` })
      }
    })
    ...
    search_data.map(({ inco_term_id }) => {
      if (sp_del_status_array.indexOf(inco_term_id) === -1) {
        sp_del_status_array.push(inco_term_id)
        option.push({ value: inco_term_id, label: getIncoTermNameByCode(inco_term_id) })
      }
    })
  } 
...
export default FMSearchSelectComponent
