...
const InvoiceAttachmentReport = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='NLFD_Invoice_Attachment_Report_'+dateTimeString
  ...
  const loadTripShipmentDeliveryReport = (cid,fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      VehicleAssignmentService.getInvoiceAttachmentReortData(cid).then((res) => {
        console.log(res,'res')
        tableReportData = res && res.data.length > 0 ? JSON.parse(res.data) : []      
        ...
        filterData.map((data, index) => { 
          rowDataList.push({
            ...
            Invoice_Attachment_Remarks: data.defect_type,
            Trip_Remarks: data.pyg_remarks,
            Accounts_Remarks: data.trip_remarks,
            Submission_Status: data.submission_status && data.submission_status == 1 ? 'Completed' : 'Not Completed',
      ...
      VehicleAssignmentService.sentInvoiceAttachmentReortDataForReport(report_form_data).then((res) => {
        console.log(res, 'res')
        // let tableReportData = res && res.data.length > 0 ? JSON.parse(res.data) : []  
        let tableReportData = res && res.data.length > 0 ? res.data : []  
