...
/* ==================== Access Part End ========================*/
  ...
  const loadVehicleReadyToTrip = () => {
    DieselIntentCreationService.getVehicleReadyToDisPaymentGeneration().then((res) => {
      setFetch(true)
      tableData = res.data.data
      let rowDataList = []
      console.log(tableData,'getVehicleReadyToDisPaymentGeneration-tableData')
      ...
      tableData.map((data, index) => {
        rowDataList.push({
          ...
          Vendor_Invoice:data.sap_vendor_invoice_no,
          Waiting_At: (
          ...
          Action: ( 
            <CButton className="btn btn-success btn-sm me-md-1">
              <Link className="text-white" to={`/DISPaymentHome/${data.id}`}>
...
function secondsToDhms(date) {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => row.Vendor_Invoice,
      sortable: true,
      center: true,
    },
    {
