...
const NlmtTSOwnExpenseClosureApprovalHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const getClosureVehiclesData = (fresh_type = '') => {
    if (fresh_type !== '1') {
      NlmtTripSheetClosureService.getVehicleReadyToOwnExpenseClosureApproval().then((res) => {
        closureData = res.data.data
        console.log(closureData,'getVehicleReadyToOwnExpenseClosureApproval')
        setFetch(true)
          ...
          rowDataList.push({
            ...
            Action: (
              <CButton className="badge" color="warning">
                <Link className="text-white" to={`/NlmtTSOwnExpenseClosureApprovalHome/${data?.nlmt_trip_in_id}`}>
                  Expense Approval
                </Link>
              </CButton>
            ),
...
export default NlmtTSOwnExpenseClosureApprovalHome
