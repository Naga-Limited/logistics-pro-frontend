...
const NlmtTSOwnExpenseClosureApproval = () => {
  ...
  function SAPSimulateProcess() {
    ...
    formData.append('TRIP_SHEET', tripInfo.tripsheet_info.nlmt_tripsheet_no)    
    formData.append('LIFNR', tripInfo.vendor_info.vendor_code)
    formData.append('FREIGHT_PAYMENT', `FREIGHT_PAYMENT:${totalChargesHire || 0}`)
    formData.append('POST_DATE', values.supplier_posting_date || currentDateVbr)     
    formData.append('PLANT', 'NLMD')

    if (tripInfo.advance_payment_info && tripInfo.advance_payment_info.vendor_tds) {

      formData.append('TAX_TYPE', tripInfo.advance_payment_info.gst_tax_type == 'Empty' ? '' : (tripInfo.advance_payment_info.gst_tax_type ? tripInfo.advance_payment_info.gst_tax_type : ''))
      formData.append('TDS', tripInfo.advance_payment_info.tds_type == '1' ? 'YES' : 'NO')
      formData.append('TDS_VALUE',  tripInfo.advance_payment_info.tds_type == '1' ? tripInfo.advance_payment_info.vendor_tds : '') 
      formData.append('HSN', tripInfo.advance_payment_info.vendor_hsn ? tripInfo.advance_payment_info.vendor_hsn : '') 

    } else {

      formData.append('TDS', values.TdsHaving == 0 ? 'NO' : 'YES')
      formData.append('TDS_VALUE', (values.TdsHaving == 0 ? '' : values.TdsHaving))
      formData.append('HSN', values.HSNtax ? values.HSNtax : '') 
      formData.append('TAX_TYPE', values.GSTtax && values.GSTtax != 'Empty' ? values.GSTtax : '') 
    ...
    if (id) {
      NlmtTripSheetClosureService.getTripSettlementInfoByParkingId(id).then((res) => {
        ...
        if (res.data.data[0] === undefined) {
          ...
          values.ded_ref = res.data.data[0].ded_ref ? res.data.data[0].ded_ref : ''
          values.ded_posting_date = res.data.data[0].ded_posting_date
            ? res.data.data[0].ded_posting_date
            : ''
          values.halt_days = res.data.data[0].halt_days ? res.data.data[0].halt_days : ''
          values.TdsHaving = res.data.data[0].tds_having ? res.data.data[0].tds_having : ''
          values.tdsType = res.data.data[0].tds_type ? res.data.data[0].tds_type : ''
          values.supplier_ref_no = res.data.data[0].supplier_ref_no ? res.data.data[0].supplier_ref_no : ''
  ...
  const [rowDatan, setRowDatan] = useState([])
  ...
  useEffect(() => {
    NlmtTripSheetClosureService.getVehicleInfoById(id).then((res) => {
      ...
      fetchedData.shipment_info ? setShipmentNumber(fetchedData.shipment_info.shipment_no) : setShipmentNumber('')

      setTripInfo(fetchedData)

      // ✅ FIX START
      const advance_total_amount_data = Number(fetchedData?.advance_payment_info?.advance_amount ?? 0)
      ...
      if (
        ...
        // values.low_tonnage_charges = fetchedData.advance_payment_info.low_tonnage_charges
        //   ? fetchedData.advance_payment_info.low_tonnage_charges
        //   : ''
      }
      ...
      let rowDataLister = []
      const Log_Info = fetchedData.tripsheet_info.freight_log_info ?? '[{}]'

      // Convert JSON string to JavaScript array
      var log_info_array = JSON.parse(Log_Info)

      console.log(log_info_array,'freight_log_info12-array')

      log_info_array.map((item, index) => {
        let log_info_object = {}
        ...
        rowDataLister.push(log_info_object) 
      })

      console.log(rowDataLister,'freight_log_info-rowDataLister')
      setRowDatan(rowDataLister)  
      ...
      console.log('freight_log_info22-rowDataLister')
  ...
  }, [id, idt.length > 0, userMasterData.length == 0, rowDatan.length == 0])
  ...
  const getOldLogInfo = () => {
    let data = [] 
    let temp = tripInfo.tripsheet_info.freight_log_info ? JSON.parse(tripInfo.tripsheet_info.freight_log_info) : []
  ...
  function FreightUpdationRejection() {
    ...
    formDataUpdate.append('id', tripInfo.tripsheet_id)
    ...
    current_info.push({
      route_id: tripInfo.tripsheet_info.route_id ?? '',
      freight_rate: tripInfo.tripsheet_info.trip_freight_rate ?? '', 
      freight_change: 1, 
      updated_freight_rate: tripInfo.tripsheet_info.trip_updated_freight_rate ?? '', 
      type: 3, /* Rejection */
      user: user_id,
      time: current_time,
      remarks: rejRemarks,
  ...
  const totalChargesCalculator = (data) => {
    let total_charge = 0
    let unload_charge = 0 
    console.log(tripInfo?.vehicle_info?.vehicle_type_id,'tttttt')

    //console.log(tripInfo, 'totalChargesCalculator-totalChargesCalculator-rjso')
    if (Number(tripInfo?.vehicle_info?.vehicle_type_id) === 22) {

      let hire_freight = totalFreightAmountFinder(tripInfo)
      total_charge =
    ...
    const roundedTotal = Number(total_charge.toFixed(2))

    console.log(roundedTotal, 'total_charge')
  ...
  const sapStatusChange = () => { 
    ...
    SAPData.append('VEHICLE_NO', tripInfo.vehicle_info ? tripInfo.vehicle_info.vehicle_number : '')
    SAPData.append('Flag', 2)

    let LPData = new FormData()
    LPData.append('tripsheet_no', tripInfo.tripsheet_info.nlmt_tripsheet_no)
    LPData.append('update_by', user_id)
    LPData.append('pyg_id', tripInfo.nlmt_trip_in_id)
    ...
    TripSheetInfoService.UpdateTSInfoToSAP(SAPData).then((response) => {
      ...
      if (response.data && (response.data[0].STATUS == 1 || response.data[0].STATUS == 2)) {
        // setFetch(true)
        toast.success(`${response.data[0].MESSAGE} for the Tripsheet : ${tripInfo.tripsheet_info.nlmt_tripsheet_no}`)
  ...
  const driverExpenseFinder = () =>{

    let toll = Number(values.toll_amount) ?? 0
    let bata = Number(values.bata) ?? 0
    let unload = Number(values.unloading_charges) ?? 0
    let others = Number(values.misc_charges) ?? 0
    let load = Number(values.loading_charges) ?? 0

    let total = toll+bata+unload+others+load
                        ...
                        <CRow className="mt-2">
                          ...
                          <CCol xs={12} md={3}>
                            ...
                            <CFormInput
                              size="sm"
                              id="vNum1"
                              value={tripInfo.vehicle_info ? tripInfo.vehicle_info.vehicle_number : ''}
                              readOnly
                            />
                          </CCol>
                          <CCol xs={12} md={3}>
