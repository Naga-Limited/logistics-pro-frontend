...
const NlmtTSHireExpApprovalHome = () => {
  ...
  const getClosureVehiclesData = (fresh_type = '') => {
    if (fresh_type !== '1') {
      NlmtTripSheetClosureService.getVehicleReadyToExpenseClosureApproval2().then((res) => {
        closureData = res.data.data
        console.log(closureData,'getVehicleReadyToExpenseClosureApproval2')
        setFetch(true)
        ...
        const filterData = filterData1.filter(
          ...
          (
            ...
            (
               data.trip_settlement_info?.tripsheet_is_settled == 1 && data.trip_settlement_info?.approval_status == 1
          ...
          rowDataList.push({
            ...
            Action: (
              <CButton className="badge" color="warning">
                <Link className="text-white" to={`/NlmtTSHireExpApprovalHome/${data?.nlmt_trip_in_id}`}>
                  Expense Approval
                </Link>
              </CButton>
            ),
