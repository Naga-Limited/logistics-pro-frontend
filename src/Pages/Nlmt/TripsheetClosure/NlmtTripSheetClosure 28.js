...
const NlmtTripSheetClosure = () => {
    ...
    if (id) {
      NlmtTripSheetClosureService.getTripSettlementInfoByParkingId(id).then((res) => {
        ...
        if (res.data.data[0] === undefined) {
          ...
          values.cost_center = res.data.data[0].cost_center ? res.data.data[0].cost_center : ''
          values.ded_remarks = res.data.data[0].ded_remarks ? res.data.data[0].ded_remarks : ''
          values.halt_days = res.data.data[0].halt_days ? res.data.data[0].halt_days : ''
  ...
  useEffect(() => {
    NlmtTripSheetClosureService.getVehicleInfoById(id)
      .then((res) => {
        ...
        // ✅ FIX START
        const advance_total_amount_data = Number(
          fetchedData?.advance_payment_info?.advance_amount ?? 0
        )
        ...
        if (
          ...
          if (
            ...
            values.freight_charges =
              Number(fetchedData.advance_payment_info.actual_freight ?? 0) -
  ...
  if (Number(tripInfo?.vehicle_info?.vehicle_type_id) === 22 && tripInfo?.advance_payment_info) {
    values.freight_charges = Number(tripInfo?.advance_payment_info?.actual_freight || 0)
  } else if (Number(tripInfo?.vehicle_info?.vehicle_type_id) === 22) {
    const billedQty = Number(tripInfo?.vehicle_assignment?.billed_qty || 0)

    const rate = Number(tripInfo?.tripsheet_info?.trip_vehicle_route?.freight_rate || 0)

    values.freight_charges = billedQty * rate
  }
  ...
  const totalChargesCalculator = (data) => {
    ...
    const freight_charges =
      ...
      tripInfo.tripsheet_info.trip_vehicle_route.freight_rate
    ...
    if (tripInfo && stoTableDataFCI && stoTableDataFCI.length > 0) {
      let fci_expenses =
        Number(data.fci_atti_cooli_charges ? data.fci_atti_cooli_charges : 0) +
    ...
    console.log(total_charge, 'total_charge+total_charge')
  ...
  // calculate additional freight
  const additionalFreight =
    Number(values.unloading_charges || 0) +
    Number(values.weighment_charges || 0) +
    Number(values.halting_charges || 0) +
    Number(values.sub_delivery_charges || 0) +
  ...
  let approval_status = additionalFreight > 0 ? 1 : 0

  const toastShown = useRef(false)

  useEffect(() => {
    if (approval_status === 1 && !toastShown.current) {
      toast.warning('Additional Freight added please get approval ..')
  ...
  const [stoValuesFCI, setStoValuesFCI] = useState(TripsheetClosureConstants.stoInitialStateFCI)
  ...
  const addEnable = (data) => {
    ...
    var hire_vehicle_check =
      tripInfo.tripsheet_info.trip_vehicle_info.vehicle_type_id == 22 ? true : false
  ...
  const imageCompress = async (event, need_data, ftype) => {
    ...
    if (ftype == 'rmsto') {
      ...
      if (file.type == 'application/pdf') {
        ...
        //yazhinisconsole.log(file.type, 'filename')
  ...
  useEffect(() => {
    if (tripInfo && tripInfo[0]?.tripsheet_info && tripInfo[0]?.trip_sheet_info.nlmt_tripsheet_no) {
      ...
      NlmtStoDeliveryDataService.getRmstoDeliveryData(
      ...
      // if(trip_sap_flag == '1'){
      NlmtStoDeliveryDataService.getAllFgstoDeliveryData(
        tripInfo.tripsheet_info.nlmt_tripsheet_no
      ).then((res) => {
        let sap_response = res.data
  ...
  useEffect(() => {
    ...
    if (stoTableData && tripInfo && tripInfo.tripsheet_info) {
      ...
      let condition2 = stoTableData.length === 0 && fgsto_tripAddonAvailability == 2 ? true : false
  ...
  useEffect(() => {
    if (stoTableDataFCI && tripInfo && tripInfo.tripsheet_info) {
      ...
      let condition4 = stoTableDataFCI.length == 0 ? true : false
  ...
  useEffect(() => {
    ...
    let bank_advance = tripInfo.advance_payment_info
      ...
      : 0
    const advance_total_amount_data = tripInfo?.advance_payment_info
      ? tripInfo?.advance_payment_info?.advance_payment
      : 0
    ...
    if (tripInfo && tripInfo.tripsheet_info) {
      ...
      VehicleRequestMasterService.getClosureVehicleRequests().then((res) => {
        ...
        let filterData = vrList.filter(
          (data) =>
            data.vr_tr_no == null || data.vr_tr_no == tripInfo.tripsheet_info.nlmt_tripsheet_no
        )
  ...
  const getOthersSapData = (e) => {
    ...
    NlmtStoDeliveryDataService.getOthersDeliveryData(
      stoOthersValues.others_sto_doc_number,
      values.ot_process_type
    )
      .then((res) => {
        setFetch(true)
        ...
        let process_type_exception_array = [
          'CREDIT MEMO NO',
          'Bill of Supply No',
          'Delivery Challan No',
          'Gate Pass NO',
        ...
        if (
          ...
          (sap_from_plant_name || sap_to_plant_name) &&
          (sap_from_plant_code || sap_to_plant_code)
        ) {
  ...
  const TripsheetClosureSubmit = (process) => {
    ...
    if (tripInfo.vehicle_info.vehicle_type_id != '3') {
      if (vrNotMappingSapDoc() == 1) {
        setFetch(true)
        toast.warning(
          `Return Journey Approval should be completed before Expense Closure Process..`
        )
        return false
    ...
    // formData.append('approval_status',Number(tripInfo?.trip_vehicle_info?.vehicle_type_id) === 22 &&
    //expenseClosureApproval? 1 : 0 )
    ...
    if (tripInfo.vehicle_info.vehicle_type_id == 21) {
      if (tripInfo.advance_info) {
        formData.append(
          'low_tonage_charges',
          tripInfo.advance_info.low_tonnage_charges ? tripInfo.advance_info.low_tonnage_charges : 0
        )
      } else {
        formData.append(
          'low_tonage_charges',
          values.low_tonnage_charges ? values.low_tonnage_charges : 0
        )
      }
    } else {
      formData.append(
        'low_tonage_charges',
        values.low_tonage_charges ? values.low_tonage_charges : 0
      )
    }
    ...
    if (tripInfo.vehicle_info.vehicle_type_id == '22') {
      ...
      if (tripInfo.advance_info && tripInfo.advance_info.vendor_tds) {
        ...
        formData.append(
          'vendor_tds',
          tripInfo.advance_info.vendor_tds ? tripInfo.advance_info.vendor_tds : ''
        )
        formData.append(
          'vendor_hsn',
          tripInfo.advance_info.vendor_hsn ? tripInfo.advance_info.vendor_hsn : ''
        )
        formData.append(
          'gst_tax_type',
          tripInfo.advance_info.gst_tax_type ? tripInfo.advance_info.gst_tax_type : ''
        )
      } else {
    ...
    formData.append(
      'vendor_id',
      tripInfo.vendor_info ? tripInfo?.vendor_info?.vendor_id : values.vendor_id
    )
    ...
    if (
      ...
      tripInfo.rj_so_info &&
      tripInfo.rj_so_info.length > 0
    ) {
      formData.append('rjso_bata_amount', values.rjso_bata_amount ? values.rjso_bata_amount : 0)
      formData.append(
        'rjso_loading_charges',
        values.rjso_loading_charges ? values.rjso_loading_charges : 0
      )
      formData.append(
        'rjso_commision_charges',
        values.rjso_commision_charges ? values.rjso_commision_charges : 0
      )
      formData.append(
        'rjso_tarpaulin_charges',
        values.rjso_tarpaulin_charges ? values.rjso_tarpaulin_charges : 0
      )
      formData.append(
        'rjso_weighment_charges',
        values.rjso_weighment_charges ? values.rjso_weighment_charges : 0
      )
      formData.append(
        'rjso_unloading_charges',
        values.rjso_unloading_charges ? values.rjso_unloading_charges : 0
      )
      formData.append('rjso_misc_charges', values.rjso_misc_charges ? values.rjso_misc_charges : 0)
      formData.append(
        'rjso_munic_charges',
        values.rjso_munic_charges ? values.rjso_munic_charges : 0
      )
      formData.append('rjso_halt_charges', values.rjso_halt_charges ? values.rjso_halt_charges : 0)
      formData.append(
        'rjso_en_diesel_charges',
        values.rjso_en_diesel_charges ? values.rjso_en_diesel_charges : 0
      )
    }
    ...
    if (tripInfo && stoTableDataFCI && stoTableDataFCI.length > 0) {
      formData.append(
        'fci_atti_cooli_charges',
        values.fci_atti_cooli_charges ? values.fci_atti_cooli_charges : 0
      )
      formData.append('fci_having', 1)
      formData.append('fci_extra_charges', values.fci_extra_charges ? values.fci_extra_charges : 0)
      formData.append(
        'fci_office_exp_charges',
        values.fci_office_exp_charges ? values.fci_office_exp_charges : 0
      )
      formData.append(
        'fci_gate_exp_charges',
        values.fci_gate_exp_charges ? values.fci_gate_exp_charges : 0
      )
      formData.append(
        'fci_weighment_charges',
        values.fci_weighment_charges ? values.fci_weighment_charges : 0
      )
    }
    ...
    if (process == 'submit') {
      ...
      console.log(stoTableDataFCI, 'stoTableDataFCI')
      if (
        tripInfo.vehicle_info.vehicle_type_id == 23 &&
        stoTableDataFCI &&
        ...
        for (var i = 0; i < stoTableDataFCI.length; i++) {
          ...
          SentVAInfoData.VENDOR_TYPE =
            stoTableDataFCI[i].sto_vendor_status_fci == 2
              ? 3
              : 1 /* 1 - Freight Vendor, 2 - Loading Vendor, 3 - Freight & Loading Vendor */
        ...
        NlmtTripSheetInfoService.StopTSInfoToSAP(SAPData)
          .then((response) => {
      ...
      // formData.append('approval_status',Number(tripInfo?.trip_vehicle_info?.vehicle_type_id) === 22 && expenseClosureApproval? 1 : 0 )
            ...
            <CTabContent className="p-3">
              <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={mainKey === 1}>
                ...
                <CNav variant="tabs" role="tablist">
                  ...
                  {tripInfo?.diesel_intent_info && tripInfo.vehicle_info.vehicle_type_id == 21 && (
                    <CNavItem>
                      <CNavLink
                        active={activeKey === 5}
                    ...
                    <CRow className="mt-2">
                      ...
                      <CCol xs={12} md={3}>
                        ...
                        <CFormInput
                          ...
                          value={
                            tripInfo && tripInfo.tripsheet_info
                              ? [1, 2].includes(Number(tripInfo.tripsheet_info.advance_request))
                                ? 'YES'
                                : 'NO'
                              : ''
                          }
                      ...
                      {tripInfo && tripInfo.vehicle_info.vehicle_type_id === 22 && (
                        <>
                          ...
                          {tripInfo?.vendor_info?.vendor_code == null && (
                            <>
                              <CCol
                                xs={12}
                                md={6}
                            ...
                            return (
                              <>
                                <CRow key={`HireshipmentChildData_${index}_${val_index}`}>
                                  ...
                                  <CCol xs={12} md={2}>
                                    <CFormLabel htmlFor="sNum">
                                      Billed Shipment Quantity in MTS
                                    </CFormLabel>
                                ...
                                <CCol xs={12} md={2}>
                                  ...
                                  <CCol xs={12} md={2}>
                                    <CFormLabel htmlFor="fjPod">FJ POD Copys</CFormLabel>
                                    <CFormInput
                                      size="sm"
                                      type="file"
                    ...
                    >
                        ...
                        <CTableBody>
                                  ...
                                  <CFormInput
                                    ...
                                    value={
                                      errors.freight_charges
                                        ...
                                        : Number(
                                            tripInfo.vehicle_assignment[0]?.billed_qty *
                                              tripInfo.tripsheet_info.trip_vehicle_route
                                                .freight_rate || 0
                                          )
                                    }
                                    readOnly
                          ...
                          <CTableRow>
                            ...
                            <>
                              ...
                              {tripInfo.advance_info &&
                              tripInfo.advance_info.low_tonnage_charges &&
                              Number(tripInfo.advance_info.low_tonnage_charges) > 0 ? (
                                <>
                                  <CFormInput
                                    size="sm"
                      ...
                      <CTable caption="top" hover style={{ height: '45vh' }}>
                        ...
                        <CTableBody>
                          ...
                          {tripInfo &&
                          tripInfo.advance_info &&
                          tripInfo.advance_info.gst_tax_type ? (
                            <CTableRow>
                              <CTableDataCell scope="row">
                                <b>4</b>
                              </CTableDataCell>
              ...
              <CTabPane role="tabpanel" aria-labelledby="profile-tab" visible={mainKey === 2}>
                ...
                <CNav variant="tabs" role="tablist">
                  ...
                  {(othersAvailable ||
                    (tripInfo && tripInfo.tripsheet_info && stoOthersTableData.length == 0)) && (
                    <>
                      <CNavItem>
                        <CNavLink
                          style={{
                ...
                <CTabContent>
                      ...
                      {tripInfo?.vehicle_info?.vehicle_type_id == 22 && (
                        <>
                          <CCol xs={12} md={3}>
                            ...
                            <CFormInput size="sm" value={tripInfo.driver_name} readOnly />
                          </CCol>

                          <CCol xs={12} md={3}>
                            <CFormLabel>Driver Mobile Number</CFormLabel>
                            <CFormInput size="sm" value={tripInfo.driver_phone_1} readOnly />
                          </CCol>
                        </>
                      )}
                      {tripInfo?.vehicle_info?.vehicle_type_id == 21 && (
                  ...
                  <CCol xs={12} md={3}>
                    ...
                    {tripInfo && tripInfo?.vehicle_info?.vehicle_type_id == 24 && (
                      <>
                        ...
                        {smallFetch &&
                          vehicleRequestsData.map((currentVrData, kk) => {
                            return (
                              <>
                                <CRow>
                                  <CCol xs={12} md={3}>
                                  ...
                                  <CCol xs={12} md={3}>
                                    <CFormLabel htmlFor="tNum">
                                      Vehicle Body Type / Variety
                                    </CFormLabel>
                                    <CFormInput
                                      size="sm"
                                      id="tNum"
                                      value={`${veh_body_finder(currentVrData.vr_body_id)} / ${
                                        veh_variety_finder(currentVrData.vr_variety_id) || '-'
                                      }`}
                                      readOnly
                                    />
                                  </CCol>
                              ...
                              <CCol xs={12} md={2}>
                                ...
                                <div className="d-grid gap-2">
                                  ...
                                  >
                                    <CNavLink
                                      style={{ color: 'blue' }}
                                      href={`${data.rj_shed_copy}`}
                                      target={'_blank'}
                                    >
                                      <i style={{ marginRight: '5px' }} className="fa fa-eye"></i>
                                      View
                                    </CNavLink>
                                  </CButton>
                                </div>
                                ...
                                {stoValuesFGSTO && stoValuesFGSTO.length > 0 && (
                                  <CCol xs={12} md={2}>
                                    ...
                                    >
                                      {console.log(
                                        stoValuesRMSTO,
                                        'stoValuesRMSTO - Own Vehicle FGSTO POD Copy - OVRPC'
                                      )}
                                      {!(
                                        stoValuesFGSTO[index_val] &&
                                        stoValuesFGSTO[index_val].sto_pod_copy_fgsto_file_upload
                                ...
                                <>
                                  <CRow className="mt-2" key={`rmsto_data_${index_val}`}>
                                    ...
                                    {stoValuesRMSTO && stoValuesRMSTO.length > 0 && (
                                      <CCol xs={12} md={2}>
                                        <CFormLabel>POD Copy</CFormLabel>{' '}
                                        {/* Own Vehicle RMSTO POD Copy - OVRPC */}
                                        <CButton
                                          className="w-100 m-0"
                                          color="info"
                                          size="sm"
                                        ...
                                        >
                                          {console.log(
                                            stoValuesRMSTO,
                                            'stoValuesRMSTO - Own Vehicle RMSTO POD Copy - OVRPC'
                                          )}
                                          {!(
                                            stoValuesRMSTO[index_val] &&
                                            stoValuesRMSTO[index_val].sto_pod_copy_rmsto_file_upload
                                        ...
                                        <CFormInput
                                          onBlur={onBlur}
                                          onChange={(e) =>
                                            handleStoFileUploadChangeRMSTO(e, index_val)
                                          }
                                          type="file"
                                          accept=".jpg,.jpeg,.png,.pdf"
                                          name={'sto_pod_copy_rmsto'}
                              ...
                              <>
                                ...
                                <CCol xs={12} md={2}>
                                  ...
                                  >
                                    <option value="">Select ...</option>
                                    {vrData.map(
                                      ({ vr_id, vr_no, vr_division, vr_purpose, request_by }) => {
                                        return (
                                          <>
                                            <option key={vr_id} value={vr_id}>
                                              {`${vr_no} (${div_finder(
                                                vr_division
                                              )} - ${purp_finder(vr_purpose)} : By ${request_by})`}
                                            </option>
                                          </>
                                        )
                                      }
                                ...
                                <CCol xs={12} md={2}>
                                  ...
                                  <CFormInput
                                    ...
                                    value={
                                      stoOthersValues.others_sto_vr_id == ''
                                        ? '--'
                                        : div_finder_by_vr_id(stoOthersValues.others_sto_vr_id)
                                    }
                                    readOnly
                                  />
                                </CCol>
                                  ...
                                  <CRow className="mt-2" key={`rmsto_data_${index_val}`}>
                                    ...
                                    <CCol md={2}>
                                      ...
                                      <CFormInput
                                        size="sm"
                                        value={
                                          data_val.sto_vendor_status_fci == 1
                                            ? 'Freight Only Vendor'
                                            : 'Freight & Loading Vendor'
                                        }
                                        readOnly
                                      />
                                    </CCol>
                  ...
                  >
                    ...
                    {Array.isArray(dieselCollectionInfo) &&
                              ...
                              <CRow className="mt-2" key={`fgstoData${index}`}>
                                <CCol md={2}>
                                  ...
                                  <CFormInput
                                    ...
                                    id="fjNum"
                                    value={
                                      fgstoData.sto_delivery_number || fgstoData.sto_delivery_no
                                    }
                                    readOnly
                                  />
                                </CCol>
                              ...
                              <CRow className="mt-2" key={`fgstoData${index}`}>
                                <CCol md={2}>
                                  ...
                                  <CFormInput
                                    ...
                                    id="fjNum"
                                    value={
                                      fgstoData.sto_delivery_number_fgsto ||
                                      fgstoData.sto_delivery_no_fgsto
                                    }
                                    readOnly
                                  />
                                </CCol>
                                ...
                                <CRow className="mt-2" key={`rmstoData${index}`}>
                                  <CCol md={2}>
                                    ...
                                    <CFormInput
                                      ...
                                      id="fjNum"
                                      value={
                                        rmstoData.sto_delivery_number_rmsto ||
                                        rmstoData.sto_delivery_no
                                      }
                                      readOnly
                                    />
                                  </CCol>
                                ...
                                <CRow className="mt-2" key={`fciData${index}`}>
                                  <CCol md={2}>
                                    ...
                                    <CFormInput
                                      size="sm"
                                      id="fjNum"
                                      value={fciData.sto_migo_number_fci}
                                      readOnly
                                    />
                                  </CCol>
                        ...
                        <CTableBody>
                          ...
                          <CTableRow>
                            ...
                            <CTableDataCell>
                              ...
                              />
                              {errors.bata && (
                                <span className="small text-danger">{errors.bata}</span>
                              )}
                            </CTableDataCell>
                            <CTableDataCell scope="row">
                              <CFormInput
                          ...
                          {tripInfo &&
                            tripInfo.trip_vehicle_info &&
                            tripInfo.tripsheet_info.trip_vehicle_info.vehicle_type_id &&
                            tripInfo.tripsheet_info.trip_vehicle_info.vehicle_type_id === 1 &&
                            tripInfo.rj_so_info &&
                            tripInfo.rj_so_info.length > 0 && (
                              <>
                                ...
                                <CTableRow>
                                  ...
                                  <CTableDataCell scope="row">
                                    <CFormInput
                                      ...
                                      value={
                                        errors.rjso_misc_charges ? 0 : values.rjso_misc_charges
                                      }
                                      readOnly
                                    />
                                  </CTableDataCell>
                                ...
                                <CTableRow>
                                  ...
                                  <CTableDataCell scope="row">
                                    <CFormInput
                                      ...
                                      value={
                                        errors.rjso_munic_charges ? 0 : values.rjso_munic_charges
                                      }
                                      readOnly
                                    />
                                  </CTableDataCell>
                                ...
                                <CTableRow>
                                  ...
                                  <CTableDataCell scope="row">
                                    <CFormInput
                                      ...
                                      value={
                                        errors.rjso_halt_charges ? 0 : values.rjso_halt_charges
                                      }
                                      readOnly
                                    />
                                  </CTableDataCell>
                  ...
                  >
                    ...
                    <CRow className="mt-2">
                      <CCol xs={12} md={3}>
                        <CFormLabel htmlFor="remarks">Remarks1</CFormLabel>
                        <CFormTextarea
                          // name="remarks"
                          id="remarks"
                          rows="1"
