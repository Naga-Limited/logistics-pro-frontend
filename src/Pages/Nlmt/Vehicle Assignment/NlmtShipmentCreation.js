...
const NlmtShipmentCreation = () => {
  ...
  const deleteShipment = () => {
    ...
    if (ship_parent_not_having && (deletedShipmentInfo.trip_sheet_info.sap_flag == '1' || deletedShipmentInfo.trip_sheet_info.sap_flag == '2')) {

      let SAPData = new FormData()
      // tripHirePaymentData.trip_sheet_info.nlmt_tripsheet_no
      ...
      LPData.append('tripsheet_no', deletedShipmentInfo.trip_sheet_info.nlmt_tripsheet_no)
      LPData.append('update_by', user_id)
      LPData.append('pyg_id', deletedShipmentInfo.parking_id)
      ...
      TripSheetInfoService.UpdateTSInfoToSAP(SAPData).then((response) => {
        ...
        if (response.data && (response.data[0].STATUS == 1 || response.data[0].STATUS == 2)) {
          // setFetch(true)
          toast.success(`${response.data[0].MESSAGE} for the Tripsheet : ${deletedShipmentInfo.trip_sheet_info.nlmt_tripsheet_no}`)
          NlmtTripSheetClosureService.nlmtUpdateSAPTripStopFlagRequest(LPData).then((res) => {
            ...
            if (res.status == 200) {   
              ...
              toast.warning('Shipment Stop Flag cannot be updated in SAP.. Kindly Contact Admin!')
              setFetch(true)
              setTimeout(() => {
                // window.location.reload(false)
              }, 1000)
