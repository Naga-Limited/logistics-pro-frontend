...
import NlmtAdvanceVehicleSelectCompont from './segments/VehicleListComponent/NlmtAdvanceVehicleList'
...
const NlmtAdvancePaymentReport = (srVehicle
  ) => {
  /*================== User Location Fetch ======================*/
...
// console.log(values.vehicle_number)
    ...
    NlmtReportService.Advancereport(tableData).then((res) => {
      setFetch(true)
...
console.log(tableData1,'tableData1')
     let rowDataList = []
      ...
      tableData1.map((data, index) => {
        ...
        rowDataList.push({
          sno: index + 1,
          Vehicle_Type: getVehicleTypeLabel(data.parking_info?.vehicle_info?.vehicle_type_id),
          ...
          tripsheet_created: data.parking_info?.tripsheet_info?.created_date,

          driver_name:data.parking_info?.vehicle_info?.vehicle_type_id === 21? data.parking_info?.driver_info?.driver_name || '-': data.parking_info?.vehicle_info?.vehicle_type_id === 22? data.parking_info?.driver_name || '-': '-',
...
export default NlmtAdvancePaymentReport
