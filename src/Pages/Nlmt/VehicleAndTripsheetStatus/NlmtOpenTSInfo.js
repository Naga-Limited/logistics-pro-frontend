...
import PygVehicleSearchSelectComponent from 'src/Pages/ParkingYardGate/CommonComponent/PygVehicleSearchSelectComponent'
...
const NlmtOpenTSInfo = () => {
  ...
  let page_no = NlmtScreenAccessCodes.NlmtTSStatusScreens.Nlmt_Vehicle_Open_Tripsheet_Info
  ...
  const onChange = (event) => {
    ...
    if (vehicle_id) {
      ...
      NlmtTripInService.getNlmtOpenTripsheetInfoByVId(vehicle_id).then((res) => { 
        let rowDataList = []
        ...
        tableData123.map((data, index) => {
          rowDataList.push({
            ...
            vehicle_number: data.vehicle_number,  
            Vehicle_Type: data.vehicle_type_id == 21 ? 'Own' : (data.vehicle_type_id == 22 ? 'Hire' : (data.vehicle_type_id == 23 ? 'Party' : '-')),
            ...
            driver_name: data.vehicle_type_id == 21 ? data.driver_name : data.hire_driver_name,
  ...
  useEffect(() => {
    ...
    NlmtTripInService.getNlmtTripInDistinctVehiclesData().then((resd)=>{
      let da = resd.data
      ...
      setFetch(true)
      console.log(test_array,'getNlmtTripInDistinctVehiclesData') 
