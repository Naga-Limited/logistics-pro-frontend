...
const NlmtTripSheetEditRequest = () => {
...
// console.log(values.driverId)
  ...
  const UpdateTripsheetVehicle = () => {
    ...
    if (currentVehicleInfo.vehicle_type_id.id != '4' && (currentVehicleInfo.trip_sheet_info.sap_flag == '1' || currentVehicleInfo.trip_sheet_info.sap_flag == '2')) {
      ...
      console.log(values.driverId)
      SAPData.append('TRIP_SHEET',currentVehicleInfo.tripsheet_info.nlmt_tripsheet_no)
      SAPData.append('VEHICLE_NO', currentVehicleInfo.vehicle_info.vehicle_number)
        ...
        if (sap_status == '1') {
          ...
          TripSheetCreationService.updateTripSheet(id,data).then((res) => {
            if (res.status == 200) {
              ...
              navigation('/NlmtTripsheetUpdation')
            }
          })
  ...
  function TripSheetCancel  () {
    ...
    if (veh_type != 'Party' && (currentVehicleInfo.tripsheet_info.sap_flag == '1' || currentVehicleInfo.tripsheet_info.sap_flag == '2')) {
      console.log('IF part')
      let SAPData = new FormData()
      SAPData.append('TRIP_SHEET',currentVehicleInfo.tripsheet_info.nlmt_tripsheet_no)
      SAPData.append('VEHICLE_NO', currentVehicleInfo.vehicle_info.vehicle_number)
      SAPData.append('VEHICLE_TYPE',veh_type.toUpperCase())
      SAPData.append('DRIVER_NAME',currentVehicleInfo.driver_name)
      SAPData.append('DRIVER_CODE', 0) 
      SAPData.append('TRUCK_CAPACITY', veh_capacity_finder(currentVehicleInfo.vehicle_info.vehicle_capacity_id)) 
      SAPData.append('DRIVER_PH_NO', currentVehicleInfo.driver_phone_1)  
      SAPData.append('Division', 'NLMT')
      SAPData.append('Stop_Flag', '4')

      let data = new FormData()
      data.append('parking_id', currentVehicleInfo.nlmt_trip_in_id)
      data.append('tripsheet_id', currentVehicleInfo.tripsheet_id)
      data.append('remarks', remarks || '')
      ...
      let data = new FormData()
      data.append('parking_id', currentVehicleInfo.parking_yard_gate_id)
      data.append('tripsheet_id', currentVehicleInfo.tripsheet_sheet_id)
      data.append('remarks', remarks || '')
      data.append('updated_by', user_id)
...
export default NlmtTripSheetEditRequest
