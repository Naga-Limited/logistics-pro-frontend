...
const NlmtFreightUpdationRequestHome = () => { 

  const exportToCSV = () => {
    let dateTimeString = GetDateTimeFormat(1)
    let fileName='Trip_Sheet_Freight_Updation_Request_Info_Report_'+dateTimeString
...
/* ==================== Access Part End ========================*/
  ...
  const loadTripShipmentReport = (fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      NlmtTSFreightUpdationService.getFreightUpdationRequestData().then((res) => {
        tableReportData = res.data.data
        console.log(tableReportData,'getFreightUpdationRequestData-tableReportData')
        setFetch(true)
        let rowDataList = []
        let filterData1 = tableReportData
        const filterData = (filterData1).filter((data) => data.parking_yard_info && data.trip_vehicle_info ) 
        console.log(filterData,'getFreightUpdationRequestData-filterData')
          ...
          rowDataList.push({
            ...
            Action: (
              <CButton className="badge" color="warning">
                <Link className="text-white" to={`${data.nlmt_tripsheet_id}`}>Freight Request</Link>
              </CButton>
            ),
          })
        })
...
export default NlmtFreightUpdationRequestHome
