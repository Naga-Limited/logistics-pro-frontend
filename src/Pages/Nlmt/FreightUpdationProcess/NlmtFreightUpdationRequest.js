...
const NlmtFreightUpdationRequest = () => {
  ...
  let page_no = NlmtScreenAccessCodes.NlmtTransactionScreens.Nlmt_Freight_Updation_Request
  ...
  function FreightUpdationSubmission() {
    ...
    if((updatedFreightRate != '' && Number(currentVehicleInfo?.tripsheet_info?.trip_freight_rate) != Number(updatedFreightRate)) || (updatedFreightRate == '' && currentVehicleInfo?.tripsheet_info?.freight_approval_status == 3)){
      change_name = 'Re Request'
    }  else {
      change_name = 'Submit'
    }
    ...
    current_info.push({ 
      ...
      freight_change: change_name == 'Re Request' ? 1 : 0, 
      updated_freight_rate: updatedFreightExists ? updatedFreightRate : currentVehicleInfo?.tripsheet_info?.trip_updated_freight_rate ?? '', 
      type: change_name == 'Re Request' ? 4 : 0, /* 4 - Re Request, 0 - No Need */
      user: user_id,
      time: current_time,
      remarks: remarks,
    })
    ...
    if(updatedFreightExists){
      formDataUpdate.append("trip_updated_freight_rate", updatedFreightRate)
      formDataUpdate.append("freight_approval_status", change_name == 'Re Request' ? 4 : 0) /* 4 - Re Request, 0 - No Need */
    }  else {
      formDataUpdate.append("trip_updated_freight_rate", currentVehicleInfo?.tripsheet_info?.trip_updated_freight_rate)
      formDataUpdate.append("freight_approval_status", 4) /* 4 - Re Request */
    }

    NlmtTSFreightUpdationService.rerequestFreightUpdationApprovalData(formDataUpdate).then((res) => {
      console.log(res)
      setFetch(true)
      if (res.status == 200) {
        Swal.fire({
          title: "Freight Updation Re Request Sent Successfully!",
          icon: "success",
          confirmButtonText: "OK",
        }).then(function () {
          navigation('/NlmtFreightRequest')
        });
        ...
        toast.warning(
          'NLMT Freight Approval Cannot Be Rerequested From LP.. Kindly Contact Admin!'
        )
      }
    })
    .catch((error) => {
  ...
  const LogType = ['','Creation','Approval','Rejection','Re-Request','Advance Completed']
  ...
  return (
    <>
                ...
                <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={true}>
                  <CForm className="container p-3" onSubmit={handleSubmit}>
                    <CRow className="">                       
                        ...
                        <CInputGroup>
                          ...
                          {!updatedFreightExists && (
                            <CButton
                              size="sm"
                              color="secondary"
                              // disabled={freightRate == 0 || freightRate == '' ? true : false}
                              onClick={(e) => {
                                // getPanData(e)
                      ...
                      {updatedFreightExists && (
                        <CCol md={3}>
                          <CFormLabel>Revised Freight Rate</CFormLabel>
                          <CInputGroup>
                            <CFormInput
                              size="sm"
                              // type="number"
                    ...
                    <CRow className="mt-2">
                      ...
                      <CCol xs={12} md={3}>
                        <CFormLabel htmlFor="dMob">Approval Remarks</CFormLabel>
                        <CFormInput
                          name="dMob"
                          size="sm"
                          id="dMob"
                          value={currentVehicleInfo?.tripsheet_info?.freight_approval_remarks}
                          readOnly
                        />
                      </CCol>
                      <CCol xs={12} md={3}>
                    ...
                    <CRow className="mt-2">
                      {(Number(currentVehicleInfo?.tripsheet_info?.trip_freight_rate) != Number(updatedFreightRate)) && updatedFreightRate != '' && (
                        <CCol md={7} style={{ border: "1px solid black", marginTop: "2px", background: "aliceblue" }}>
                          ...
                          <span style={{ display: "block", fontWeight: "bold" }} className="big text-danger">
                            If the revised freight differs from the original freight, manager approval is required.
                          </span>
                        </CCol>
                      )}
                      {(updatedFreightRate == '' && currentVehicleInfo?.tripsheet_info?.freight_approval_status == 3) && (
                        <CCol md={7} style={{ border: "1px solid black", marginTop: "2px", background: "aliceblue" }}>
                          <CFormLabel>
                            Note :
                          </CFormLabel>
                          <span style={{ display: "block", fontWeight: "bold" }} className="big text-danger">
                            If the modified freight differs from the original freight, manager approval is required.
                          </span>
                        </CCol>
                      )}
                    </CRow>
                    <CRow className="mt-2">
                      ...
                      > 
                        {((Number(currentVehicleInfo?.tripsheet_info?.trip_freight_rate) != Number(updatedFreightRate)) && updatedFreightRate != '' || (updatedFreightRate == '' && currentVehicleInfo?.tripsheet_info?.freight_approval_status == 3)) ? (
                          <CButton
                            size="sm"
                            color="success"
      ...
      <CModal visible={confirmBtn} onClose={() => setConfirmBtn(false)}>
        <CModalBody>
          <p className="lead">{`Are You sure To ${((Number(currentVehicleInfo?.tripsheet_info?.trip_freight_rate) != Number(updatedFreightRate)) && updatedFreightRate != '' || (updatedFreightRate == '' && currentVehicleInfo?.tripsheet_info?.freight_approval_status == 3)) ? 'Re Request' : 'Submit'} this freight updation ? `}</p>
        </CModalBody>
        <CModalFooter>
...
export default NlmtFreightUpdationRequest
