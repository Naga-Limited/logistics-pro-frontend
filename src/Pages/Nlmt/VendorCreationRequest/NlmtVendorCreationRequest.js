...
const NlmtVendorCreationRequest = () => {
  ...
  let page_no = NlmtScreenAccessCodes.NlmtVendorCreationScreens.Nlmt_Vendor_Request
  ...
  const vendorInfoUpdate = (sap_pan_data) => {
    ...
    formData.append('existing_vendor', 1)
  ...
  const addVendorRequest = (status) => {
    ...
    formData.append('vc_request_remarks', values.vc_request_remarks || '')
    formData.append('vc_request_by', user_id)
  ...
  useEffect(() => {
    const fetchDefinitions = async () => {
      try {
        // First API call
        const taxTypeResponse = await DefinitionsListApi.visibleDefinitionsListByDefinition(3);
        let tableData = taxTypeResponse?.data?.data || [];

        const filterData = tableData.filter(
          (data) => Number(data.definition_list_status) === 1
        );

        setTaxType(filterData);

        // Second API call
        const gstTaxTermsResponse = await DefinitionsListApi.visibleDefinitionsListByDefinition(20);
        let viewData = gstTaxTermsResponse?.data?.data || [];
              ...
              <CCard>
                <CForm className="container p-3" onSubmit={handleSubmit}>
                  ...
                  <CRow className="">
                    ...
                    <CCol xs={12} md={3}>
                      <CFormLabel htmlFor="vc_request_remarks">
                        Remarks 
                      </CFormLabel>
                      <CFormInput
                        size="sm"
                        id="vc_request_remarks"
                        name="vc_request_remarks"
                        value={values.vc_request_remarks ? values.vc_request_remarks : ''}
                        onFocus={onFocus}
                        onBlur={onBlur}
                        onChange={handleChange}
                      />
                  ...
                  <CRow>
                    <CCol xs={12} md={6}>
                      <Link to="/NlmtVendorRequestTable">
                        <CButton
                          md={9}
                          size="sm"
                          color="primary"
