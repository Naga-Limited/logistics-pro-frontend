...
/* ==================== Access Part End ========================*/
  ...
  const loadTripShipmentReport = (fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      NlmtTSCreationService.getNlmtTripInfoCancelReport().then((res) => {
        ...
        const filterData = (filterData1).filter((data) => data.vehicle_info )
        ...
        filterData.map((data, index) => {
          rowDataList.push({
            sno: index + 1, 
            // Creation_Date: formatDate(data.created_date),
            Creation_Date: data.created_date,
            Creation_Month: getDateTime(data.created_date,2), 
            ...
            Waiting_At: (
              <span className="badge rounded-pill bg-info">
                {data.vehicle_current_position == 8
                  ? 'Doc. Verify'
            ...
            Action: (
              <CButton className="badge" color="warning">
                <Link className="text-white" to={`${data.nlmt_trip_in_id}`}>Trip Cancel</Link>
              </CButton>
            ),
          })
        })
...
export default NlmtTripCancelHome
