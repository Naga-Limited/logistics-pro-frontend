...
const NlmtAdvancePaymentApprovalHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const loadVehicleReadyToTrip = (fresh_type = '') => {
    if (fresh_type !== '1') {
      NlmtAdvanceCreationService.getVehicleReadyToAdvanceApproval().then((res) => {
        setFetch(true)
        ...
        const filterData = filterData1.filter(
          (data) =>
            (data.advance_payment_info && data.advance_payment_info.approval_status == 1)
        )
...
export default NlmtAdvancePaymentApprovalHome
