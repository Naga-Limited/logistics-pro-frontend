...
import FSSearchSelectComponent from '../NLFS/Reports/FSSearchSelectComponent'
export const nlfs_diesel_vendor_code = process.env.REACT_APP_NLFS_DIESEL_VENDOR

const NLFSDieselIndentSequenceCreation = () => {
  /*================== User Location Fetch ======================*/
  ...
  let page_no = LogisticsProScreenNumberConstants.NLFSDieselIntentModule.FS_DIS_No_Creation_NTS
  ...
  const totalFuelQuantity = (data) => {
    ...
    data.map((val,ind)=>{ 
      fuel_quantity += Number(parseFloat(val.fuel_quantity).toFixed(3))
    })
  ...
  const fuelTo = ['','Vehicle','Barel','Others','Car']
  ...
  function checkModalDisplay(type=0) {
    ...
    if(diInfo.di_orders.length > 1 && differentDivisionAssign(diInfo.di_orders)){
      toast.warning('FSB number creation is not allowed for diesel indents belonging to different divisions..!')
      setAssignPSModal(false)
      return false
    }

    if(diInfo.di_orders.length > 1 && differentDieselToAssign(diInfo.di_orders)){
      toast.warning('FSB number creation is not allowed for diesel fillings belonging to different purposes..!')
      setAssignPSModal(false)
      return false
    }
    ...
    if(type==0){
      if (Object.keys(diInfo.response).length > 0) {
        ...
        toast.warning('Please Choose Atleast One Diesel Indent for FSB Number Creation !')
  ...
  useEffect(()=>{
    NLFSDieselIntentService.getNLFSVehicleReadyToDieselIndentSequenceNumberGeneration().then((res) => {
      setFetch(true)
  ...
  const [reportDivision, setReportDivision] = useState(0) 
  ...
  const onChangeFilter = (event, event_type) => {
    ...
    } else if (event_type == 'division') {
      if (selected_value) {
        setReportDivision(selected_value)
      } else {
        setReportDivision(0)
      }
    }  
  ...
  const fsbFilterDataSubmission = () => {
    // setFetch(true) 
    console.log(reportVehicle,'fsbFilterDataSubmission-reportVehicle')
    console.log(reportVehicleType,'fsbFilterDataSubmission-reportVehicleType')
    console.log(reportDivision,'fsbFilterDataSubmission-reportDivision')

    console.log(dateRangePickerStartDate, 'fsbFilterDataSubmission-start date')
    console.log(dateRangePickerEndDate, 'fsbFilterDataSubmission-end date')
    ...
    // if(dateRangePickerStartDate == '' && dateRangePickerEndDate == '' && reportVehicle == 0 && reportVehicleType == 0 && reportDivision == 0){
    ...
    report_form_data.append('division', reportDivision)

    NLFSDieselIntentService.filterNLFSVehicleReadyToDieselIndentSequenceNumberGeneration(report_form_data).then((res) => {
      console.log(res, 'res')
      setFetch(true) 
      let tableData = res.data.data 
  ...
  const DieselIndentSubmission = () => {
    ...
    tripDIInfoArray.map((map_data,map_index)=>{
      trip_di_no_info.push({
        id: map_data.id,
        vehicle_number: map_data.vehicle_no || map_data.carry_vehicle,
        di_no: map_data.di_no, 
      })
    ...
    let fuel_to = tripDIInfoArray[0].diesel_to == '1' ? 'V' : (tripDIInfoArray[0].diesel_to == '2' ? 'B' : (tripDIInfoArray[0].diesel_to == '3' ? 'O' : 'C'))
    ...
    di_submission_data.append('division_id', tripDIInfoArray[0].division_info.division_id) /* 5-Logistics Division */
    ...
    di_submission_data.append('type', 2) /* 1-Others */ 
    di_submission_data.append('fuel_to', fuel_to)
              ...
              <CCard className="p-1">                 
                <CContainer className="m-2">
                  <CRow className="mt-1 mb-1">
                    ...
                    <CCol xs={12} md={3}>
                      <CFormLabel htmlFor="VNum">Division</CFormLabel>
                      <FSSearchSelectComponent
                        size="sm"
                        className="mb-2"
                        onChange={(e) => {
                        ...
                        search_type="fs_fsb_filter_division"
                    ...
                    <CCol xs={12} md={3}>
                      ...
                      <FSSearchSelectComponent
                        ...
                        search_type="fs_fsb_filter_vehicle_type"
                        search_data={searchFilterData}
                      />
                    </CCol>
                    <CCol xs={12} md={3}>
                      ...
                      <FSSearchSelectComponent
                        ...
                        search_type="fs_fsb_filter_vehicle_number"
                        search_data={searchFilterData}
                      />
                    </CCol>
                    {/* <CCol xs={12} md={3}>
                                  ...
                                  <>
                                    <CTableRow>
                                      ...
                                      <CTableDataCell
                                        ...
                                        scope="row"
                                      >
                                        {formatDate(data.diesel_invoice_date)}
                                      </CTableDataCell>
                                      <CTableDataCell
                                        style={{ width: '9%', textAlign: 'center' }}
                                        scope="row"
                                      >
                                        {fuelTo[data.diesel_to]}
                                      </CTableDataCell>
                                      <CTableDataCell
                                        style={{ width: '9%', textAlign: 'center' }}
                                        scope="row"
                                      >
                                        {data.division_info ? data.division_info.short_name : '-'}
                                      </CTableDataCell>
                                      <CTableDataCell
                                        style={{ width: '9%', textAlign: 'center' }}
                                        scope="row"
                                      >
                                        {data.vehicle_no || data.carry_vehicle}
                                      </CTableDataCell>
                                      {/* <CTableDataCell
                                        style={{ width: '9%', textAlign: 'center' }}
                                        scope="row"
                                      ...
                                      <CTableDataCell
                                        ...
                                        scope="row"
                                      >
                                         {data.old_amount ? parseFloat(data.old_amount - data.total_amount).toFixed(2) : 0}
                                      </CTableDataCell>
                                      <CTableDataCell
                                        style={{ width: '9%', textAlign: 'center' }}
                                        scope="row"
              ...
              <CRow className="">
                ...
                <CCol md={3}>
                  ...
                  <CFormInput
                    ...
                    size="sm"
                    id="cmn"
                    value={`${tripDIInfoArray[0].division_info.division_name} (${tripDIInfoArray[0].division_info.short_name})`}
                    readOnly
                  />
                </CCol>
                
                <CCol md={2}>
                  <CFormLabel htmlFor="cmn">Diesel Indent Count</CFormLabel>
                  <CFormInput
                    style={{fontWeight: 'bolder'}}
                    name="cmn"
                    size="sm"
              ...
              <CTable striped hover style={{height:tripDIInfoArray.length > 2 ? '40vh' : '20vh'}}>
                <CTableHead className='mt-3' style={{background: 'pink'}}>
                  <CTableRow>
                    ...
                    <CTableHeaderCell scope="col" style={{width: '15%', textAlign:"center"}}>DI No.</CTableHeaderCell>
                    <CTableHeaderCell scope="col" style={{width: '15%', textAlign:"center"}}>Fuel To</CTableHeaderCell>
                    <CTableHeaderCell scope="col" style={{width: '15%', textAlign:"center"}}>Vehicle No.</CTableHeaderCell>
                    <CTableHeaderCell scope="col" style={{width: '15%', textAlign:"center"}}>User Name</CTableHeaderCell>
                    ...
                    <CTableHeaderCell scope="col" style={{width: '15%', textAlign:"center"}}>Discount Rate</CTableHeaderCell>
                  ...
                  {tripDIInfoArray.map((val,ind)=>{
                    ...
                    return(
                      <CTableRow key={ind}>
                        ...
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{val.di_no}</CTableDataCell>
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{fuelTo[val.diesel_to]}</CTableDataCell>
                        {/* <CTableHeaderCell scope="col" style={{width: '30%'}}>{val.contractor_info.contractor_name}</CTableHeaderCell> */}
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{val.vehicle_no || val.carry_vehicle}</CTableDataCell>
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{val.vehicle_user_name}</CTableDataCell>
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{val.fuel_quantity}</CTableDataCell>
                        ...
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{val.sap_fuel_rate}</CTableDataCell>
                        <CTableDataCell scope="col" style={{width: '15%', textAlign:"center"}}>{val.old_amount ? parseFloat(val.old_amount - val.total_amount).toFixed(2) : 0}</CTableDataCell>
...
export default NLFSDieselIndentSequenceCreation
