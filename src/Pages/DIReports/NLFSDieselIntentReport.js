...
import NlfsDieselSelectComponent from './Segments/DieselVehicleNumber/NlfsDieselVehicleList'

const NLFSDieselIntentReport = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.NLFSDieselIntentModule.FS_DI_Info_Report
  ...
  const [fuelFor, setFuelFor] = useState(0)
  ...
  const onChangeFilter = (event, event_type) => {
    ...
    } else if(event_type == 'fuel_for'){
      if (selected_value) {
        setFuelFor(selected_value)
      } else {
        setFuelFor(0)
      }
    }
  ...
  const loadVehicleReadyToTrip = () => {
    ...
    tableData.append('fuel_for', fuelFor)  
      
    NLFSDieselIntentService.NlfsDieselReport(tableData).then((res) => {
      setFetch(true)
      ...
      tableData1.map((data, index) => {
        rowDataList.push({
          sno: index + 1,
          Vehicle_Type: vehType[data.diesel_to],
          vehicle_number: data.vehicle_id == 0 ? data.carry_vehicle : data.vehicle_info?.vehicle_no,
          trip_sheet_no: data.di_no,
          tripsheet_created: data.created_at, 
          ...
          rate_of_ltrs: data.sap_fuel_rate,
          no_of_ltrs: data.fuel_quantity,
          total_amount: data.total_amount, 
          diesel_vendor_sap_invoice_no: data.diesel_vendor_sap_invoice_no,
          di_status: DISTATUS[data.status],
          sap_status: data.nlfs_sap_status ? SAPSTATUS[data.nlfs_sap_status] : '-',
          di_creation_time: data.created_at_format,
  ...
  const DISTATUS = ['','Created','Confirmed','Approved','Cancelled']
  const SAPSTATUS = ['','SO Created','DO Created','Invoice Created','E-Way Bill Generated','DIS No Genereated','Vendor Invoice Generated','Vendor Payment Generated','Receipt Generated']
  ...
  const columns = [
    ...
    {
      name: 'Diesel status',
      selector: (row) => row.di_status,
      sortable: true,
      center: true,
    },
    {
      name: 'SAP status',
      selector: (row) => row.sap_status,
      sortable: true,
      center: true,
    },
    // {
              ...
              <CCard className="mt-4">
                <CContainer className="mt-1">
                  <CRow className="mt-1 mb-1" >
                    ...
                    <CCol xs={12} md={3}>
                      <CFormLabel htmlFor="VNum">
                        Diesel Indent Number
                      </CFormLabel>
                      <NlfsDieselSelectComponent
                        size="sm-lg"
                        className="mb-2"
                        onChange={(e) => {
                          onChangeFilter(e, 'trip_sheet_no')
                        }}
                        label="Diesel Indent Number"
                        id="trip_sheet_no"
                        name="trip_sheet_no"
                    ...
                    <CCol xs={12} md={3}>
                      ...
                      <NlfsDieselSelectComponent
                        ...
                        className="mb-1"
                        onChange={(e) => {
                          onChangeFilter(e, 'fuel_for')
                        }}
                        label="Diesel For"
                        id="diesel_for"
                        name="diesel_for"
                        // value={values.vehicle_number}
                        search_type="fuel_for"
                        search_data={searchFilterData}
                      />
                    </CCol>
                    <CCol xs={12} md={3}>
                    ...
                    <CCol xs={12} md={3}></CCol>
                    <CCol xs={12} md={3} className='mt-4' style={{display:'flex',justifyContent:'end'}}>
                      <CButton
                        size="lg-sm"
                        color="primary"
...
export default NLFSDieselIntentReport
