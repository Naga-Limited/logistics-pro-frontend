...
const DieselApprovalHire = ({
  ...
  useEffect(() => {
    // if(singleVehicleInfo.acc_appr_status == 0)
    // {
  ...
  useEffect(() => {
    ...
    if(values.sap_invoice_diesel_posting_date && values.diesel_vendor_id == 3){
...
const netAmountFinder = (tot,type) => {
  ...
  return type == 1 ? Math.round(discount_amount) : dis_amount
}
    ...
    <>
      ...
      <CRow>
        ...
        {singleVehicleInfo.nlfs_sap_status == null ? (
          <>
            ...
            <CCol xs={12} md={3}>
              <CFormLabel htmlFor="bunk_reading">{`${values.diesel_vendor_id == 3 ? 'Dispensary Bill' : 'Bunk Reading'}`}</CFormLabel>
                <CButton className="w-100 m-0" color="info" size="sm" id="inputAddress">
                  {!fileuploaded ? (
                      <>
      ...
      <CModal
        ...
        >
          <CModalHeader>
            <CModalTitle>
              {`${values.diesel_vendor_id == 3 ? 'Signed Diesel Indent / Manual Bill Photo' : 'Invoice Copy Photo'}`}
            </CModalTitle>
          </CModalHeader>
          <CModalBody>
