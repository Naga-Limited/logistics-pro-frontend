...
/* ==================== Access Part End ========================*/
  ...
  const NLFS_SAP_STATUS = ['','SO ✔️', 'DO ✔️', 'Invoice ✔️']
  ...
  const [dieselVendorList, setDieselVendorList] = useState([])
    ...
    DieselIntentCreationService.getVehicleReadyToDieselApproval().then((res) => {
      ...
      const filterData1 = tableData.filter(
        (data) => 
          ...
          data.acc_appr_status != 1 && 
          (data.nlfs_sap_status == null || data.nlfs_sap_status < 3)
      )
      console.log(filterData1,'getVehicleReadyToDieselApproval-filterData1')
      ...
      filterData1.map((data, index) => {
        rowDataList.push({
          ...
          Diesel_Vendor: data.vendor_code ? dieselVendorInfoByCode(data.vendor_code) :  '-',
          Acc_Approval: data.acc_appr_status == 2 ? 'Approved ✔️' : (data.acc_appr_status == 3 ? 'Rejected ❌' : '-'),
          ...
          NLFS_STATUS : data.vendor_code == nlfs_diesel_vendor_code ? (data.nlfs_sap_status ? NLFS_SAP_STATUS[data.nlfs_sap_status]: '❌') : '-',
          DICR_Date : formatDate(data.created_at),
          DICO_Date : data.confirmed_at ? formatDate(data.confirmed_at) : '-',
          Action: (
            <CButton className="badge text-white" color="warning">
              <Link to={`${data.id}`}>DI Approval</Link>
            </CButton>
          ),
        })
      })
...
function secondsToDhms(date) {
  ...
  const columns = [
    ...
    {
      name: 'DI Date',
      selector: (row) => row.DICR_Date,
      sortable: true,
      center: true,
    },
    {
      name: 'Fill Date',
      selector: (row) => row.DICO_Date,
      sortable: true,
      center: true,
    },
    {
    ...
    {
      name: 'Diesel Vendor',
      selector: (row) => row.Diesel_Vendor,
      sortable: true,
      center: true,
    },
    {
    ...
    {
      name: 'Acc. Approval',
      selector: (row) => row.Acc_Approval,
      sortable: true,
      center: true,
    },
    {
      name: 'NLFS Status',
      selector: (row) => row.NLFS_STATUS,
      sortable: true,
      center: true,
    },
    {
  ...
  const dieselVendorInfoByCode = (code) => {
    let vendor_name = '-'
    console.log(dieselVendorList,'dieselVendorInfoByCode-dieselVendorList')
    console.log(code,'dieselVendorInfoByCode-code')
    dieselVendorList.map((vv,kk)=>{
      if(vv.vendor_code == code){
        vendor_name = vv.diesel_vendor_name
      }
    })
    console.log(vendor_name,'dieselVendorInfoByCode-vendor_name')
    return vendor_name
  }
  ...
  const loadNLFSDICreation = () => {
    NLFSDieselIntentService.getVehicleReadyToDieselApproval().then((res) => {
      ...
      const filterData1 = view_data.filter(
        (data) => ((data.acc_appr_status != 1) && data.nlfs_sap_status != 4 && !(data.nlfs_sap_status == 3 && data.diesel_to != 2))
      )
      console.log(view_data,'getVehicleReadyToDIConfirm-filtered')
      filterData1.map((data, index) => {
        rowDataList.push({
          sno: index + 1,
          DI_No: data.di_no,
          DI_Date: data.created_at, 
          DIC_Date: formatDate(data.confirmed_at), 
          ...
          NLFS_STATUS : data.nlfs_sap_status ? NLFS_SAP_STATUS[data.nlfs_sap_status]: '❌',
          Action: ( 
            <CButton className="badge text-white" color="warning">
              <Link to={`/OtherDIApproval/${data.id}`}>DI Approval</Link>
            </CButton> 
          )
        })
      })
  ...
  useEffect(() => {
    ...
    DieselIntentCreationService.getDieselVendor().then((res) => {
      setDieselVendorList(res.data.data)
    })
  }, [divisionData.length == 0,plantData.length == 0,dieselVendorList.length == 0, rowData.length == 0]) 
