...
export const driverPodImageUrlValidation = (link, type = '') => {
  ...
  if (typeof link?.name === 'string') {
    return type === 'url' ? link.name : true;
  }

  // ✅ URL STRING
  if (typeof link === 'string' && link.length > 0) {
    let idx = link.lastIndexOf("/");
    let fileName = link.substring(idx + 1);

    return type === 'url' ? fileName : !!fileName;
  }
...
export const getCurrentDateTime = () => {
  ...
  let ans = `${day}-${month}-${year} ${hours}:${minutes} ${ampm}`
  console.log(ans,'getCurrentDateTime')
