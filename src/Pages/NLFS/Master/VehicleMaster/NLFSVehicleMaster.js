...
const NLFSVehicleMaster = () => {
  ...
  function addNewVehicle() {
    ...
    if(vehicleAlreadyExistInVehicleMaster(values.vechile_number) === 1){
    ...
    setFetch(false)
    NLFSVehicleMasterApi.createNLFSVehicles(formData).then((res) => {
      setFetch(true)
      if (res.status === 201) {
  ...
  useEffect(() => {
    ...
    VehicleVarietyService.getVehicleVariety().then((res) => {
      console.log(res.data.data,'setVehicleVariety')
