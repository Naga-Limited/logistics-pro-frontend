...
const DIVendorInvoiceCreationHome = () => {
  /*================== User Location Fetch ======================*/
...
/* ==================== Access Part End ========================*/
  ...
  const loadVehicleReadyToTrip = () => {
    DieselIntentCreationService.getVehicleReadyToVendorInvoiceCreation().then((res) => {
      setFetch(true)
      tableData = res.data.data
      let rowDataList = []
      console.log(tableData,'getVehicleReadyToVendorInvoiceCreation-tableData')
        ...
        rowDataList.push({
          ...
          Action: ( 
            <CButton className="btn btn-success btn-sm me-md-1">
              <Link className="text-white" to={`/DIInvoiceCreationHome/${data.id}`}>
...
function secondsToDhms(date) {
  ...
  const loadNLFSDICreation = () => {
    NLFSDieselIntentService.getNLFSVehicleReadyToDieselIndentVendorInvoiceGeneration().then((res) => {
      ...
      view_data.map((data, index) => {
        rowDataList.push({
          ...
          Action: ( 
            <CButton className="btn btn-success btn-sm me-md-1">
              <Link className="text-white" to={`/OtherDIInvoiceCreation/${data.id}`}>
                <i className="fa fa-eye" aria-hidden="true"></i>
              </Link>
            </CButton>
          ),
       ...
       <>
        {screenAccess ? (
         <>
          ...
          <CCard>
            <CContainer>
              <h3 style={{marginTop:'15px'}}>Non Tripsheets - NLLD Division</h3>
              <CustomTable
                columns={columns1}
                data={row1Data}
...
export default DIVendorInvoiceCreationHome
