...
const NLFSDIVendorInvoiceCreation = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='NLFS_DI_Diesel_Info_Report_'+dateTimeString
  ...
  useEffect(() => {
    ...
    NLFSDieselIntentService.getNLFSDISInfoById(id).then((res) => {
      ...
      diesel_indent_info_data.map((data, index) => {
        ...
        rowDataList.push({
          S_NO: index + 1,             
          Tripsheet: data.di_no, 
          Vehicle_Type: data.diesel_to == 1 ? 'Vehicle' : (data.diesel_to == 2 ? 'Barel' :  (data.diesel_to == 4 ? 'Car' : 'Others')),
          Vehicle_No: payment_info_id_data.trip_di_no_info[index].vehicle_number,
          // Driver_Name: data.driver_info.driver_name,
          Diesel_Quantity: data.fuel_quantity,
          Diesel_Rate: data.sap_fuel_rate,
  ...
  const validationApproval = () => {
    ...
    for (var i = 0; i < di_array_length; i++) {
      ...
      HireDeduction.VEHICLE_NO = singleContractorArray.trip_di_no_info[i].vehicle_no
      HireDeduction.TRIPSHEET_NO = singleContractorArray.trip_di_no_info[i].di_no
      HireDeduction.REFERENCE = ''
      HireDeduction.RATE_PER_LTR = singleContractorArray.di_id_info_objects[i].sap_fuel_rate
      HireDeduction.NO_OF_LTR = singleContractorArray.di_id_info_objects[i].fuel_quantity
    ...
    DieselIntentCreationService.disSapVendorInvoiceCreation(SAPApendData_Seq).then( 
      ...
      if (
        ...
        sap_invoice_reference == singleContractorArray.dis_no
          ...
          NLFSDieselIntentService.NLFSVendorInvoiceCreation(form_data).then((res)=>{
...
export default NLFSDIVendorInvoiceCreation
