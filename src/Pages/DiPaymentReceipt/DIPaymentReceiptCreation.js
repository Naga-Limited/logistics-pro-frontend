...
const DIPaymentReceiptCreation = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='NLFS_Receipt_Diesel_Info_Report_'+dateTimeString
  ...
  const deleteTripAllPayment = () => {
    ...
    NLFSDieselIntentService.rejectAllNLFSTripDiData(formDataUpdate).then((res) => {
      console.log(res)
      setFetch(true)
      if (res.status == 200) {
        Swal.fire({
          ...
          navigation('/DIPaymentReceiptHome')
        });
      ...
      NLFSDieselIntentService.getNLFSDISInfoById(id).then((res) => {
        ...
        diesel_indent_info_data.map((data, index) => {
          ...
          rowDataList.push({
            ...
            Amount: payment_info_id_data.type == 1 ? (data.driver_id == 0 ? data.total_amount: data.old_amount): data.old_amount,
            Discount_Rate: payment_info_id_data.type == 1 ? (data.driver_id == 0 ? '-' : data.old_amount) : (data.old_amount ? data.old_amount - data.total_amount: 0),
  ...
  useEffect(() => {  
    if(singleContractorArray && singleContractorArray.dis_no && !(singleContractorArray.type == 2 && singleContractorArray.status == 1)){  
  ...
  const validationApproval = () => {
    ...
    if (invoicePostingDate) {
      ...
      setFetch(true)
      toast.warning('You should select receipt posting date before submitting..!')
      return false
    }
    ...
    for (var i = 0; i < di_array_length; i++) {
      ...
      HireDeduction.INVOICE = invoiceNoFinder(singleContractorArray.di_id_info_objects[i]) 
    ...
    SAPApendData.KUNNR = singleContractorArray.division_info.add_col_one
    // SAPApendData.BANK_PAYMENT = singleContractorArray.fuel_amount
    SAPApendData.BANK_PAYMENT = singleContractorArray.sap_fuel_amount ? singleContractorArray.sap_fuel_amount : singleContractorArray.fuel_amount
    ...
    // setFetch(true)
    // return false
    DieselIntentCreationService.nlfsDisSapPaymentReceiptCreation(SAPApendData_Seq).then( 
      (res) => {
      let sap_invoice_no = res.data.CUS_RECEIPT_NO
      let sap_invoice_status = res.data.REC_STATUS
      let sap_invoice_message = res.data.REC_MESSAGE
      ...
      if (
        ...
        sap_invoice_reference == singleContractorArray.dis_no
          ...
          form_data.append('customer_code', singleContractorArray.division_info.add_col_one) 
          form_data.append('payment_remarks', values.apremarks)
          ...
          DieselIntentCreationService.sendNLFSPaymentReceiptCreation(form_data).then((res)=>{
          console.log(res,'sendNLFSPaymentReceiptCreation')
          setFetch(true)
          if (res.status == 200 && res.data.lp_status == 1 && res.data.invoice_reference == sap_invoice_no) {
            Swal.fire({
              title: 'DIS Payment Receipt Created Successfully!',
              icon: "success",
              text:  'Payment Receipt : ' + sap_invoice_no,
              confirmButtonText: "OK",
            }).then(function () {
              navigation('/DIPaymentReceiptHome')
            });
            ...
            toast.warning('DIS Payment Receipt Created in SAP But there is an issue in LP Updation. Kindly contact Admin..!')
          }
        })
        .catch((errortemp) => {
        ...
        setFetch(true)
        toast.warning('DIS Payment Receipt Creation Failed in SAP.. Kindly Contact Admin!')
      }
    })
    .catch((errortemp) => {
      console.log(errortemp)
                  ...
                  <CRow className="m-2">
                    ...
                    <CCol md={3}>
                      ...
                      <CFormInput
                        ...
                        size="sm"
                        id="cmn"
                        value={`${singleContractorArray.division_info?.short_name} - ${singleContractorArray.division_info?.division_name}`}
                        readOnly
                      />
                    </CCol>               
                    ...
                    <CCol md={3}>
                      <CFormLabel htmlFor="cmn">Total Amount / DI Count </CFormLabel>
                      <CFormInput
                        style={{fontWeight: 'bolder'}}
                        name="cmn"
                        size="sm"
                    ...
                    {
                      singleContractorArray.status == 3 && (
                        <>
                          <CCol md={3}>
                          ...
                          <CCol md={3}>
                            <CFormLabel htmlFor="cmn"> Payment Amount </CFormLabel>
                            <CFormInput
                              style={{fontWeight: 'bolder'}}
                              name="cmn"
                              size="sm"
                              id="cmn"
                              value={`${singleContractorArray.sap_fuel_amount ? singleContractorArray.sap_fuel_amount : singleContractorArray.fuel_amount}`}
                              readOnly
                            />
                          </CCol> 
    
                          <CCol md={3}>
                            <CFormLabel htmlFor="cmn"> Payment Doc. No </CFormLabel>
                            <CFormInput
                              style={{fontWeight: 'bolder'}}
                              name="cmn"
                              size="sm"
                              id="cmn"
                              value={`${singleContractorArray.sap_payment_doc_no}`}
                              readOnly
                            />
                          </CCol> 
                          ...
                          <CCol md={3}>
                            ...
                            <CFormInput
                              ...
                              size="sm"
                              id="cmn"
                              value={`${singleContractorArray.payment_remarks}`}
                              readOnly
                            />
                          </CCol>                 
                        </>
                ...
                <CRow className="m-2">
                  ...
                  {/* <CCol xs={12} md={3}>
                    <CFormLabel htmlFor="invoiceCopy">
                      Receipt Attachment {invoiceCopyAvailable ? '' : <REQ />}
                    </CFormLabel>
                    <CFormInput
                ...
                <CRow className="mt-2">
                  <CCol className="m-2" xs={12} sm={12} md={3}>
                    <CButton size="sm" color="primary" className="text-white" type="button">
                      <Link className="text-white" to="/DIPaymentReceiptHome">
                        Previous
                      </Link>
                    </CButton>
                  </CCol>
...
export default DIPaymentReceiptCreation
