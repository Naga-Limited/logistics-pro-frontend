...
import ParkingYardGateService from 'src/Service/ParkingYardGate/ParkingYardGateService'
  ...
  const OVTICInfoEdit = () => {
    ...
    const gettripHirePaymentData = (e) => {
      ...
      const tripsheet_regex = /^[A-Za-z]{2}\d{9}$/;

      if(tripsheetNumberNew == ''){
        setFetch(true)
        toast.warning('Tripsheet Number Required.')
        return false
      }

      if(tripsheet_regex.test(tripsheetNumberNew)){
        //
      } else {
        setFetch(true)
      ...
      if(/^[a-zA-Z0-9]+$/i.test(tripsheetNumberNew) && /[a-zA-Z]/.test(tripsheetNumberNew) && /[0-9]/.test(tripsheetNumberNew)){
        ...
        TripInfoCaptureService.getTICInfoByTripsheetNo(tripsheetNumberNew).then((res) => { 
          setFetch(true)
